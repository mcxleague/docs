---
title: Introduction
description: "vercl.co is the open-source link management infrastructure for modern marketing teams."
---

{/* src="/images/thumbnail.png" */}

<img
  className="rounded-lg border border-gray-100"
  src="https://dub.co/_static/thumbnail.png"
  alt="vercl.co Thumbnail"
/>

## Features

Here are some of the features that Dub offers:

- [Advanced Analytics](https://vercl.co/features/analytics)
- [Branded Links](https://vercl.co/features/branded-links)
- [QR Codes](https://vercl.co/features/qr-codes)
- [Personalization](https://vercl.co/features/personalization)
- [Team Collaboration](https://vercl.co/features/collaboration)

## Tech Stack

Dub's codebase is [fully open-source](https://dub.sh/github) and is built on top of the following technologies:

- [Next.js](https://nextjs.org/) – framework
- [TypeScript](https://www.typescriptlang.org/) – language
- [Tailwind](https://tailwindcss.com/) – CSS
- [Upstash](https://upstash.com/) – redis
- [Tinybird](https://tinybird.com/) – analytics
- [PlanetScale](https://planetscale.com/) – database
- [NextAuth.js](https://next-auth.js.org/) – auth
- [BoxyHQ](https://boxyhq.com/enterprise-sso) – SSO/SAML
- [Turborepo](https://turbo.build/repo) – monorepo
- [Stripe](https://stripe.com/) – payments
- [Resend](https://resend.com/) – emails
- [Vercel](https://vercel.com/) – deployments

## Getting Started

To get started with Dub, you can do any of the following:

<CardGroup cols={2}>
  <Card title="Create a free account" icon="code" href="https://app.vercl.co">
    Create a free account to get started with Dub
  </Card>
  <Card title="Try our API" icon="code" href="/api-reference/introduction">
    Learn how to use Dub's API to create and manage links programmatically
  </Card>
  <Card title="Self-host Dub" icon="building" href="/self-hosting/guide">
    Learn how to self-host Dub for greater control and privacy
  </Card>
  <Card title="Integrate with Dub" icon="circle-nodes" href="/integrations">
    Learn how to integrate Dub with your favorite tools
  </Card>
</CardGroup>
